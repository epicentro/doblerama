 ...
 * @author Yasser
 */
class cursos_word extends CI_Controller {
    //put your code here
     
    public $tabla="cursos_word";
    public $idtabla="idcursos_word";
    public $carpeta_img="word_cursos";
    
    public $tabla_padre="cursos";
    public $idtabla_padre="idcursos";
    ...
    public function download(){
        ...
        $strsql="select imagen from $this->tabla where $this->idtabla='$id'";;
        ...
        $ruta = base_url().'word_cursos/'.$archivo;
        $data['ruta']=$ruta;

        $this->load->view("be/includes_be/v_down_word", $data);
//$this->load->view("v_down", $data);         
    ...
    public function listar(){
            ...
            $strsql="select $this->idtabla, imagen, orden, idsw, idprimera_foto,nombre  from $this->tabla 
   ...
   public function imagen_pf(){
    ...
    public function v_imagen_nombre(){
       ...
       $strsql="select nombre from $this->tabla where $this->idtabla='$idtabla'";
       $fila=$this->modelo_base->c_una_fila($strsql);
       $data["nombre"]=$fila->nombre;
       
       
      
       $this->load->view("be/v_".$this->tabla."_imagen_nombre.php", $data);
   ...
   public function imagen_nombre(){
       ...
       //AVERIGUAMOS EL NOMBRE
       $strsql="select imagen from $this->tabla where $this->idtabla='$idtabla'";
       $fila=$this->modelo_base->c_una_fila($strsql);
       $imagen=$fila->imagen;
       //tipo de archivo
       $typeFile=explode('.',$imagen);
       $typeFile=$typeFile[count($typeFile)-1];
       // Renombrar
       $renombrar=$this->be_funciones->nombre_raya_abajo($nombre);

       rename ('./'.$this->carpeta_img.'/'.$imagen, './'.$this->carpeta_img.'/'.$renombrar.'.'.$typeFile);

       $newFile=$renombrar.'.'.$typeFile;
       $strsql="update $this->tabla set nombre='$nombre',imagen='$newFile' where 
                $this->idtabla='$idtabla'";
       ...
       if($valor==1){
                             ...
                             $data['recarga_padre']="si";
                             $this->load->view("be/v_".$this->tabla."_imagen_nombre", $data);
      ...
      $this->validacion->validacion_login();
       ...
       $aleatorio=random_string('alnum', 5);   
        ...
        if ( ! $this->upload->do_upload("imagen")){//En esta linea es que subimos la imagen
            ...
            $idcursos_word=$this->uri->segment(5);
            if($idcursos_word=="0"){//si es igual a cero es nuevo registro

                    //Ya tenemos creado la imagen y el Thumbnails, ahora grabamos
                    
                    $strsql="insert into $this->tabla ( imagen, idcursos )values ('$file','$id')";
                    $valor=$this->modelo_base->ejecuta($strsql);
                    
            }else{//si $idcursos_word no es cero entonces es una actualizacion
                
                
                    //ANTES DE GRABAR BORRAMOS EL LAS IMAGENES ANTERIORES
                    $strsql="select imagen from $this->tabla where id$this->tabla='$id' and idcursos_word='$idcursos_word'";
                    ...
                    //ACTUALIZAMOS
                    $strsql="update $this->tabla set imagen='$file', idcursos='$id' where 
                            idcursos_word='$idcursos_word'";
                                ...
                                if ( ! $this->image_lib->resize()){//En esta linea redimensionamos la imagen a 170 x 220
                                    ...
                                    echo $this->image_lib->display_errors();
                                        ...
                                        if($idcursos_word=="0"){//si es igual a cero es nuevo registro
                                                ...
                                                //ANTES DE GRABAR BORRAMOS EL LAS IMAGENES ANTERIORES
                                                $strsql="select imagen from $this->tabla where id$this->tabla='$idcursos_word' and idcursos='$id'";
